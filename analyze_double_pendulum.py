...
def extract_h_from_path(path):
    match = re.search(r'double_pendulum_(\d+\.?\d*)', path)
    if match:
...
def compute_energy(traj):
    # Approximate double pendulum Hamiltonian (unit mass/length/gravity):
    θ1, θ2 = traj[:, 0], traj[:, 1]
    p1, p2 = traj[:, 2], traj[:, 3]
    # Simplified 2-link planar pendulum energy for small mass ratios (m1 = m2 = 1, l1 = l2 = 1)
    delta = θ2 - θ1
    kinetic = 0.5 * p1**2 + 0.5 * p2**2 - p1 * p2 * np.cos(delta)
    potential = -2 * np.cos(θ1) - np.cos(θ2)
...
def analyze_models(G_paths, HNN_paths, LHI_paths):
    ...
    for i, (g, hnn, lhi) in enumerate(zip(G_paths, HNN_paths, LHI_paths)):
        axes[i, 0].text(-0.25, 0.5, f'Problem_id = {i+4}', transform=axes[i, 0].transAxes,
        ...
        for j, (label_base, path) in enumerate(zip(['G-SympNet', 'HNN', 'LHI'], [g, hnn, lhi])):
            ...
            if j == 0:
                ...
                axes[i, 1].plot(np.arange(start_idx, len(true)), true[start_idx:, 3],
                                label='Ground Truth', linestyle='--', color="black")

            axes[i, 0].plot(np.arange(start_idx, len(true)), pred[start_idx:, 1],
                            label=f"{label}", alpha=0.6)
            axes[i, 1].plot(np.arange(start_idx, len(true)), pred[start_idx:, 3],
                            label=f"{label}", alpha=0.6)
    ...
    for col, title in zip(range(3), ["Pendulum 1 Position", "Pendulum 2 Position", "Global Trajectory Error"]):
    ...
    plt.tight_layout()
    plt.savefig("double_pendulum_analysis_panel.png", dpi=300)
...
if __name__ == '__main__':
    ...
    G_paths = [
        base_path + 'outputs/double_pendulum_0.5_G', base_path + 'outputs/double_pendulum_0.75_G', base_path + 'outputs/double_pendulum_1_G',
    ]
    HNN_paths = [
        base_path + 'outputs/double_pendulum_0.5_HNN', base_path + 'outputs/double_pendulum_0.75_HNN', base_path + 'outputs/double_pendulum_1_HNN',
    ]
    LHI_paths = [
        base_path + 'outputs/double_pendulum_0.5_LHI', base_path + 'outputs/double_pendulum_0.75_LHI', base_path + 'outputs/double_pendulum_1_LHI',
    ]
